<template>
	<view class="wrap">
		<view class="u-tabs-box" style="padding: 20upx 20upx;">
			<u-search @search="getList" placeholder="搜干货" :disabled="false" shape="square" :show-action="false" :animation="true" :clearabled="true"></u-search>
		</view>
...
export default {
	...
	methods: {
		...
		getAllFeed(value){
			...
			setTimeout(() => {
				_this.list[0].loadStatus = search.has_more ? 'loadmore' : 'nomore'
				_this.list[0].cursor = search.cursor
				_this.list[0].list = _this.list[0].list.concat(search.data)
			})
		},
		// tab栏切换
		change(index) {
